...
The examples utilize the \proglang{R} \citep{R2020} package \pkg{spatstat} \citep{spatstat2015} in addition to the \pkg{GET} package \citep{MyllymakiMrkvicka2020}. The plots are produced by the use of the \pkg{ggplot2} package \citep{Wickham2016}, where we utilize the theme \code{theme_bw} for this document.
%
<<r_setup>>=
library("GET")
library("spatstat")
...
%
<<workflow4, fig=TRUE, width=3, height=3>>=
plot(res)
@
%
\end{itemize}
...
%
<<spruces_csr_continues, fig=TRUE, width=3, height=3>>=
...
%
<<spruces_randomlabeling2, fig=TRUE, width=3, height=3>>=
...
@
%
Then perform simulations of CSR and calculate the $L$-functions:
%
<<combined2>>=
...
%
<<combined6, fig=TRUE, width=5, height=5>>=
...
%
<<goodness-of-fit3, fig=TRUE, width=3, height=3>>=
simulations <- NULL
...
The adjusted test of the \pkg{GET} package is described in Section 2.3 of \citet{MyllymakiMrkvicka2020}. The procedure was suggested by \citet{BaddeleyEtal2017} and extended for global envelopes in \citet{MyllymakiMrkvicka2020}.
%
<<saplings>>=
data(saplings)
plot(saplings)
...
%
<<spatialF_bei, fig=TRUE, width=6, height=3>>=
fullmodel <- ~ grad
reducedmodel <- ~ 1
...
%
<<spatialF_forestfires, fig=TRUE, width=6, height=3>>=
...
%
<<saplings_matclust2, fig=TRUE, width=3, height=3>>=
...
%
<<saplings_matclust3, fig=TRUE, width=3, height=3>>=
