...
\end{align*}
%
If the probability to have ties among $T_{ik}, i=1, \ldots s$, is zero, then the probability of ties among $C_i$ is zero as well. If ties appear among $T_{ik}, i=1, \ldots s$, $T_{[i-1]k} < T_{[i]k} = \dots = T_{[j]k} < T_{[j+1]k}$, then the continuous rank is defined as $c_{[l]k} = \frac{i+j}{2} - \frac{1}{2}$ for $l = i, \dots, j$.
