...
A simulation study that will be published as a separate study "Comparison of non-parametric global envelopes" gives guidance on the required number of simulations under different scenarios \citep[see also the supplement S2 of][]{MyllymakiEtal2017}.
...
Then the choice of the measure plays a role. Based on our experience supported by the simulation study "Comparison of non-parametric global envelopes", the \code{'erl'} and \code{'area'} measures are typically good choices for the integral type of extremeness where vector $\TT_i$ is extreme in the set of vectors for a large range of its components. On the other hand, for a maximum type of extremeness, i.e., the case where $\TT_i$ is extreme only for a few of its components, the \code{'cont'} and \code{'qdir'} measures are typically the best choices, but also the \code{'area'} measure performs well. Thus, if no particular type of extremeness is expected a priori, the \code{'area'} measure is often a good compromise, since it is sensitive to the amount of outlyingness (similarly as \code{'erl'}) and to the value of outlyingness (similarly as \code{'cont'} and \code{'qdir'}).
