...
These functions utilize \fct{central\_region} and \fct{global\_envelope\_test} for the global envelope construction. In addition, many of these functions take care of preparing the simulations or permutations for the specific task.
The \fct{print()} and \fct{plot()} methods are available for the objects obtained as the output of the global envelope methods of \pkg{GET}. The latter provides the graphical interpretation of the result.
...
o <- order(forder(csets, measure = 'area'))
col3 <- c("#21908CFF", "#440154FF", "#5DC863FF")
p1 <- plot(cset1, idx = o[1:3], col_idx = col3) +
  scale_x_continuous(name = "Age (years)") +
  scale_y_continuous(name = "Height")
p2 <- plot(cset2, idx = o[1:3], col_idx = col3) +
  scale_x_continuous(name = "Age (years)") +
  scale_y_continuous(name = "Change")
p1 + p2 + plot_layout(guides = "collect")
@
	\caption{The heights (left) and height differences (right) of the 54 girls of the \code{growth} data of the \proglang{R} package \pkg{fda} at ages from 1 to 18. Three girls having most extreme curves (joint ordering by the area measure) are highlighted with the colors specified in the legend.}
...
plot(res, xlab = "Age (years)", ylab = "Value")
@
%
...
for(i in 1:nsim) {
  ...
  sim[, i] <- Lest(sim.X, correction = "translate",
                   r = r)[['trans']] - r
}
@
%
...
df <- data.frame(Height = c(fm10.l$Girls, fm14.l$Girls, fm10.l$Boys, fm14.l$Boys),
                 Gender = c(rep("Girls", times=2*nG),
                            rep("Boys", times=2*nB)),
                 Age = c(rep("Age 10", times=nG), rep("Age 14", times=nG),
                         rep("Age 10", times=nB), rep("Age 14", times=nB)))
ggplot(df, aes(Height, colour = Gender)) +
  facet_wrap("Age") + stat_ecdf() +
...
p1 <- plot(res10, xlab=substitute(paste(i, " (", italic(j), ")", sep=""),
                                  list(i="Height", j="x")))
p2 <- plot(res14, xlab=substitute(paste(i, " (", italic(j), ")", sep=""),
                                  list(i="Height", j="x")))
combined <- p1 + p2 & theme(legend.position = "bottom")
combined + plot_layout(guides = "collect")
@
...
ggplot(df) + geom_line(aes(x = Hour, y = NOx, group = Date, col = Colour)) +
...
res.c <- graph.fanova(nsim = 2999, curve_set = cset,
    groups = Type, variances = "unequal", contrasts = TRUE)
