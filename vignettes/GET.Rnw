...
\title{GET: Global Envelopes in \proglang{R}}
\Plaintitle{GET: Global envelopes in R}
\Shorttitle{Global envelopes in \proglang{R}}
...
Intrinsic graphical interpretation property is introduced for global envelopes, and the global envelopes included in the \pkg{GET} package that have the property are described and compared.
Examples of different uses of global envelopes and their implementation in the \pkg{GET} package are presented, including global envelopes for single and several one- or two-dimensional functions, Monte Carlo goodness-of-fit tests for simple and composite hypotheses, comparison of distributions, graphical functional analysis of variance (ANOVA), and general linear model (GLM), and confidence bands in polynomial regression.
}
...
economics \citep{MrkvickaEtal2018}, 
...
medicine and 
neuroscience \citep{RafatiEtal2016, LeeEtal2017, MollerEtal2019, BiscioMoller2019, MrkvickaEtal2019b}.
...
For these tasks, in addition to a global envelope, a Monte Carlo $p$~value is computed according to the measure $M_i$: $p =  \sum_{i=1}^{s} \1 (M_i \leq M_1)  \big/ s$  \citep[see, e.g.,][]{MyllymakiEtal2017, MrkvickaEtal2017, MrkvickaEtal2018, MrkvickaEtal2019b, MrkvickaEtal2019a}.
...
\item \citet{AldorNoimanEtal2013} presented a global envelope for a Q-Q plot. The shape of the envelope is derived theoretically, but the size of the envelope has to be computed from simulations. The code for this global envelope can be found at \url{http://www-stat.wharton.upenn.edu/~sivana/QConBands.r}.
The methods of the \pkg{GET} package can be used for this purpose as well, both for simple and composite hypotheses, but the theoretical achievement for simple hypotheses has apparent advantages. 
...
\end{Code}
%
where the multivariate or functional data are provided in \code{curve_sets}, \code{type} specifies type of the global envelope (see Table~\ref{tab:GEoverview} and descriptions in Section~\ref{sec:GEdef}), and the coverage or level of the global envelope is specified by \code{coverage} or \code{alpha} ($=1-$\code{coverage}), respectively.  
Additionally, one can, for example, specify the one or two-sided \code{alternative}, i.e., whether only small or large values of $\TT_i$ or both should be considered extreme.
...
Different options are supported for the data in \code{curve_sets} (see help files of the functions and examples below), but the basic form is a \code{curve_set} object that can be constructed by the function \fct{create\_curve\_set} simply providing the argument values and observed and/or simulated curves in a list (see Section~\ref{sec:GOT} for an example).
In addition, to constructing global envelopes from a set of curves, the \fct{central\_region} and \fct{global\_envelope\_test} functions provide combined central regions or combined global envelope tests (see Section~\ref{sec:combinedGE}) if the user provides in \code{curve_sets} a list consisting of (appropriate) sets of curves.

The \pkg{GET} package also provides other functions for specific tasks (see Table~\ref{tab:FUNCTIONSoverview} and the examples in Section \ref{sec:illustrations}). 
These functions utilize \fct{central\_region} and \fct{global\_envelope\_test} for the global envelope construction. In addition, many of these take care of preparing the simulations or permutations for the specific task.
...
\code{'erl'}          & \citet{MyllymakiEtal2017, NarisettyNair2016, MrkvickaEtal2018}    & Global rank envelope corresponding to extreme rank length (ERL) measure \\ 
\code{'cont'}         & \citet{Hahn2015, MrkvickaEtal2019b} & Global rank envelope corresponding to the continuous rank measure \\
\code{'area'}         & \citet{MrkvickaEtal2019b}    &  Global rank envelope corresponding to the area measure \\
...
\fct{graph.fanova}  & One-way ANOVA tests for functional data with graphical interpretation \citep{MrkvickaEtal2018} \\
\fct{frank.fanova}  & One-way functional ANOVA tests based on the global envelopes applied to $F$-values \citep{MrkvickaEtal2018} \\
...
\fct{frank.flm} & $F$-rank functional GLM, i.e., global envelope tests applied to $F$-values in permutation inference for the GLM \citep{MrkvickaEtal2019b} \\ 
...
\fct{GET.variogram} & Variogram and residual variogram with global envelopes \\\hline
\end{tabular}
...
%
\[
\mathbf{R}_i^\prime \prec \mathbf{R}_{i} \quad \Longleftrightarrow\quad
  \exists\, n\leq d: R_{i^\prime[k]} = R_{i[k]} \forall\, k < n,\  R_{i^\prime[n]} < R_{i[n]}.
\]
%
The division by $s$ scales the values to interval from 0 to 1.
...
\end{equation}
%
see \citet{NarisettyNair2016} and \citet{MrkvickaEtal2018}.

\subsubsection{Global continuous rank envelope}

The ties can alternatively be broken by the continuous rank measure \citep{Hahn2015, MrkvickaEtal2019b} which refines the extreme rank measure by considering instead of the (discrete) pointwise ranks $R_{ik}$ continuous pointwise ranks $C_{ik}$ defined by the ratios of $T_{ik}$ to the closest other $T_{jk}$, $j=1,\dots,s$, $j\neq i$.

Formally, the continuous rank measure is
%
$$C_i = \frac{1}{s_C}\min_{k=1,\ldots d} C_{ik},$$
%
where $s$ is the number of functions and $s_C = \lceil s/2 \rceil$ serves as scaling in the two-sided case, while in the one-sided case $s_C=s$.  
...
\end{align*}
%
If the probability to have ties among $T_{ik}, i=1, \ldots s$, is zero, then the probability of ties among $C_i$ is zero as well. If ties appear among $T_{ik}, i=1, \ldots s$, $T_{[i-1]k} < T_{[i]k} = \dots = T_{[j]k} < T_{[j+1]k}$, then the continuous rank is defined as $c_{[l]k} = \frac{i+j}{2} - \frac{1}{2}$ for $l = i, i+1, \dots, j$. 
...
Another refinement of rank envelope is the area rank measure \citep{MrkvickaEtal2019b},  
%
%$$A_i = \frac{1}{s_A} \sum_k \min(R_i, c_{ik}).$$
$$A_i = \frac{1}{s_A}\left(R_i - \sum_k (R_i-C_{ik}) \1(C_{ik} < R_i)\right),$$
%
where the scaling factor $s_A = \lceil s/2 \rceil d$ in the two-sided case, and $s_A=s\cdot d$ in the one-sided case.
...
On the other hand, any other choice is also fine, because the \code{'rank'}, \code{'erl'}, \code{'cont'} and \code{'area'} measures lead to an equivalent outcome for a large number of simulations or permutations. However, the definition of {\em large} depends on the situation. The study in Appendix of the first version of this paper available at \url{https://arxiv.org/abs/1911.06583v1} and to be published elsewhere gives some guidance on the required number of simulations under different scenarios \citep[see also the supplement S2 of][]{MyllymakiEtal2017}.
...
Then the choice of the measure plays a role. Based on our experience supported by the simulation studies, the \code{'erl'} and \code{'area'} measures are typically good choices for the integral type of extremeness where vector $\TT_i$ is extreme in the set of vectors for a large range of its components. On the other hand, for a maximum type of extremeness, i.e., the case where $\TT_i$ is extreme only for a few of its components, the \code{'cont'} and \code{'qdir'} measures are typically the best choices, but also the \code{'area'} measure performs well. Thus, if no particular type of extremeness is expected a priori, the \code{'area'} measure is often a good compromise, since it is sensitive to the amount of outlyingness (similarly as \code{'erl'}) and to the value of outlyingness (similarly as \code{'cont'} and \code{'qdir'}) \citep[see also][]{MrkvickaEtal2019b}.
...
The \proglang{R} package \pkg{fda} contains Berkeley Growth Study data \citep{RamsaySilverman2006} of the heights of 39 boys and 54 girls from ages 1 to 18 and the ages at which the data were collected.
...
@
%	\includegraphics[width=\textwidth]{pictures/girls_heights_and_der.pdf}
	\caption{The heights (left) and height differences (right) of the 54 girls of the \code{growth} data of the \proglang{R} package \pkg{fda} at ages from 1 to 18.}
...
By using the functional boxplot \citep{SunGenton2011} with the same measure, an investigation can be made into whether the most extreme girls are outliers with respect to height or its change. Figure~\ref{fig:girls_fboxplot} shows the 50\% central region and the functional boxplot with an inflation factor 1.5 jointly for the heights and their changes obtained by:
%
\begin{figure}[ht!]
\centering
%
<<girls_fboxplot, echo=TRUE, fig=TRUE, height=3.0625, width=6.125>>==
res <- fBoxplot(csets, type = 'area', factor = 1.5)
plot(res, xlab = "Age (years)", ylab = "")
@
%
\caption{The functional boxplot (light grey area) using the 50\% central region (dark grey area) based on the \code{'area'} measure and the expansion factor 1.5 jointly for the heights and changes of heights of the 54 girls in Figure~\ref{fig:girls_curves}. The black line is an observed data function (vector) that goes outside the functional boxplot (the 15th girl in the data).}
	\label{fig:girls_fboxplot}
\end{figure}
%

One can see that one of the girls (the 15th girl in fact) is an outlier, because she has grown extraordinarily much in her sixth year. However, the highest height curve of Figure~\ref{fig:girls_curves} is not regarded as an outlier with the given specifications.
The plot was produced by the plot method of \pkg{GET} that utilizes the \pkg{ggplot2} package \citep{Wickham2016}.
...
Figure~\ref{fig:adulttrees} shows the locations of 67 large trees (with height $> 25$ m) in an area of size $75\text{ m}\times 75\text{ m}$ from an uneven aged multi-species broadleaf nonmanaged forest in Kaluzhskie Zaseki, Russia \citep{GrabarnikChiu2002, Lieshout2010}. The point pattern is available in the \pkg{GET} package under the name \code{adult_trees}, which is a \code{ppp} object of the \pkg{spatstat} package.

\begin{figure}[htb]
\centering
<<trees, echo=FALSE, fig=TRUE, height=3.0625, width=3.0625>>==
library("spatstat")
data("adult_trees")
X <- adult_trees
...
plot(X, main="")
@
%	\includegraphics[width=0.5\textwidth]{pictures/adulttrees.pdf}
...
\centering
%
<<trees_CSR, echo=TRUE, fig=TRUE, height=3.0625, width=3.0625>>==
res <- global_envelope_test(cset, type = "erl")
plot(res, ylab = expression(italic(hat(L)(r)-r)))
@
%
	\caption{The global envelope test for the CSR of the tree pattern of Figure~\ref{fig:adulttrees} using the centred $L$-function. The grey area represents the 95\% global (\code{'erl'}) rank envelope.}
	\label{fig:trees_CSR}
\end{figure}
%
...
<<adjtest_log, echo=FALSE, fig=FALSE, height=4, width=8>>==
...
grid.arrange(p1, p2, nrow = 1)
@
%	\includegraphics[width=0.5\textwidth]{pictures/NOx_normalitytest.pdf}\includegraphics[width=0.5\textwidth]{pictures/logNOx_normalitytest.pdf}
...
Figure~\ref{fig:ecdfs} shows the empirical cumulative distribution functions obtained by the \fct{ecdf} of the \proglang{R} package \pkg{stats} \citep{R2020} for the heights of the 54 girls and 39 boys of the \code{growth} data 
\citep[see above, and][]{RamsaySilverman2006} 
at ages 10 (left) and 14 (right). 
...
Here it is assumed that the heights in the group $i$ are an i.i.d.\ sample from the distribution $F_i(r)$, $i=1, \dots, n$, and the hypothesis $F_1(r)=\dots = F_n(r)$ needs to be tested.
...
To test the equality of distributions, one simply needs to provide the samples as a list (code for age 14 shown here):
%
<<necdfs_10, echo=FALSE>>==
set.seed(190503)
fm10.l <- list(Girls=growth$hgtf["10",], Boys=growth$hgtm["10",])
res10 <- GET.necdf(fm10.l, nsim=1999)
@
%
<<necdfs_14_seed, echo=FALSE>>==
set.seed(190503)
@
%
<<necdfs_14>>==
...
@
%
and plot the object returned by \fct{GET.necdf}. Figure~\ref{fig:ecdfs_means_GET} shows the test output of the comparison of girls and boys at ages 10 (left) and 14 (right).
The height distributions at age 10 do not differ from each other significantly, but at age 14 the boys are taller, particularly with a difference that the proportion of girls reaching a height of around 175 cm is much lower.

\begin{figure}[ht!]
    \centering
<<necdfs, echo=FALSE, fig=TRUE, height=3.0625, width=6.125>>==
...
@
%	\includegraphics[width=\textwidth]{pictures/ecdfs.pdf}
	\caption{The empirical cumulative distribution functions heights of the 54 girls (black) and 39 boys (grey) of the \code{growth} data of the \proglang{R} package \pkg{fda} at ages 10 (left) and 14 (right).}
	\label{fig:necdfs}
\end{figure}
...
grid.arrange(p1, p2, nrow=1)
@
	\caption{Global envelope tests for comparison of the empirical cumulative distribution functions of heights of the girls and boys (see Figure~\ref{fig:ecdfs}). Left: Age 10; Right: Age 14.}
...
The data are available in \code{poblenou[['nox']]} which is a \code{fdata} object of the \proglang{R} package \pkg{fda.usc}.
...
@
%	\includegraphics[width=\textwidth]{pictures/NOx.pdf}
\caption{The NO$_x$ levels for Monday-Thursday (Mon-Thu), Friday (Fri) and non-working days (Free) including weekend and festive days in Poblenou for 115 days from 23 February to 26 June, 2005.}
	\label{fig:NOx_data}
\end{figure}

Assuming that the NO$_x$ levels $T_{ij}(r)$ at times $r \in R = [0,24]$ are i.i.d.\ samples from stochastic processes $SP(\mu_j, \gamma_j)$ with mean functions $\mu_j(r)$, $r\in R$, and covariance functions $\gamma_j(s, t)$, $s, t \in R$, for $j=1, \ldots , J$, the groups of NO$_x$ levels can be compared by means of the graphical functional ANOVA \citep{MrkvickaEtal2018}. The hypothesis 
%
\begin{equation*}
...
\end{equation}
%
These test functions (Equations~\ref{eq:graphfanova_means} and \ref{eq:graphfanova_contrasts}) can be obtained by the \fct{graph.fanova} function setting \code{contrasts = FALSE} and \code{TRUE}, respectively.
...
To sample from the null hypotheses, the simple permutation of raw functions among the groups is performed. The permutations and the global envelope test can be done by the \fct{graph.fanova} function:
%
<<graphfanova_seed, echo=FALSE>>==
set.seed(190503)
@
%
<<graphfanova_test, fig=FALSE>>==
...
@
%
The \code{fdata} object \code{poblenou[['nox']]} is automatically converted to the required format in \pkg{GET}. The observed differences together with the global envelope are shown in Figure~\ref{fig:logNOx_graphfanova}. 
...
The graphical functional ANOVA allows one to detect either a) which groups deviate from the mean (option \code{contrasts = FALSE}) or b) which specific groups are different (option \code{contrasts = TRUE}). The example above was for the latter. Note that this test directly has the nature of a post hoc test. Furthermore, both versions of the test allow one to identify which $r$ values lead to the potential rejection of the null hypothesis.

\begin{figure}[ht!]
	\centering
<<graphfanova_logNOx, echo=FALSE, fig=TRUE, height=2.722, width=6.125>>==
plot(res.c, xlab = "Hour", ylab = "Diff.", base_size=10)
@
	\caption{The output of the graphical functional ANOVA to test the difference between the type of the day on the log NO$_x$ levels in the Poblenou data. The observed differences between the group means and the 95\% global envelope.}
	\label{fig:graphfanova_logNOx}
\end{figure}

When a graphical interpretation for group specific differences is not of interest but the area of rejection is, instead of \fct{graph.fanova} it is possible to apply the one-way functional ANOVA based on the $r$-wise $F$-statistics, $r\in R$. This test is implemented in the function \fct{frank.fanova}. For the log NO$_x$ data, the test result was that there are differences between the groups for the whole day time (figure omitted).
...
The data considered here and available as the data object \code{abide_9002_23} in the \pkg{GET} package contains data from one of the 116 different anatomical regions in the brain partitioning being based on the anotomical automatic labeling system of \citet{Tzourio-MazoyerEtal2002}. The studied region is the right Crus Cerebellum 1 region of the brain at one slice (23) accompanied with three subject-specific factors, i.e.\ type (autism and control), sex and age. 
...
@
%	\includegraphics[width=0.75\textwidth]{pictures/abide_9002_23_subj1and27.pdf}
...
$$
%
where $\beta_i(r)$ are the elements of the $\beta(r)$. For a continuous factor of interest $k=1$. For a discrete factor of interest, $k$ is equal to the number of groups of the discrete factor, adding the additional condition that $\sum_i \beta_{i}(r)=0$ for all $r\in\{1, \dots d\}$. For interaction of a continuous and a discrete factor, $k$ is also equal to the number of groups of the categorical factor, adding the same additional condition. For the interaction of two discrete factors, $k$ is equal to the product of the numbers of groups of the discrete factors, adding the same additional condition.
For a discrete factor, two options are available for the test vector by the argument \code{contrasts} of the function \fct{graph.flm}: either \code{FALSE} (default) taking as the test statistic $\beta_i(r)$ for all $r$ and $i=1 \ldots k$, or \code{TRUE} for $\beta_i(r) - \beta_j(r)$ for all $r=1,\ldots,d$, $i\neq j$, $1\leq i < j \leq k$. The former allows to detect which groups deviate from the zero, while the latter test specifies which specific groups are different  in the post hoc nature.
Note that this also holds for interaction terms.
Furthermore, all the options allow one to identify which of the components of the vector, $r\in\{1,\ldots,d\}$, lead to the potential rejection of the null hypothesis.
Permutations under the null hypothesis are obtained using the Freedman-Lane procedure \citep{FreedmanLane1983, MrkvickaEtal2019b, MrkvickaEtal2019a}.
...
@
%
As mentioned earlier, the data are given in the arguments \code{curve_sets} and \code{factors}. Further important arguments are \code{formula.full} and \code{formula.reduced}, which specify the full GLM and the GLM where the interesting factor has been dropped out, and the number of simulations \code{nsim}. Further arguments to \fct{global\_envelope\_test} can be passed in \code{GET.args}, e.g., the type of the global envelope.
...
@
%
are two-dimensional as well (Figure~\ref{fig:abide_graphflm}).
Here only two groups were compared, and the plot shows that the brain measurements were lower in the autism group than in the control group in a part of the small example region (red locations in Figure~\ref{fig:abide_graphflm}).
...
plot(res)
@
%	\includegraphics[width=\textwidth]{pictures/abide_graphflm.pdf}
	\caption{Graphical functional GLM for testing the effect of the group (autism, control) in the brain image example: the observed difference (autism-control), the lower and upper bounds of the 95\% global rank (ERL) envelope, and the significant regions (red) where the observed coefficient goes below or above the envelope.}
...
In the $F$-rank GLM, the same linear model (Equation~\ref{eq:GLM}) is fitted at each $r\in\{1,\ldots,d\}$ and permutations under the null hypothesis are obtained similarly by the Freedman-Lane procedure as in the graphical functional GLM \citep{FreedmanLane1983, MrkvickaEtal2019b}. 
...
@
%
Figure~\ref{fig:abide_frankflm} shows the test result of the $F$-rank GLM, which found significant differences between the groups approximately at the same pixels $r\in\{1,\ldots,d\}$ of the brain image as the graphical functional GLM above.
In general, for a factor with more than two groups, the $F$-rank GLM is however not able to tell between which specific groups of a discrete factor the differences occur (or which of the groups deviates from the mean).
...
plot(res.F)
@
	\caption{$F$-rank functional GLM for testing the effect of the group (autism, control) in the brain image example: the observed $F$~statistic, the upper bound of the one-sided 95\% global rank (ERL) envelope, and the significant region (red) where the observed $F$-statistic exceeds the envelope.}
...
and the \fct{central\_region} function was applied to this set to obtain the 95\% global confidence band (using the type \code{'erl'}).
...
Besides the graphical interpretation, another advantage of the proposed global rank envelopes is their non-parametric (rank-based) nature, which ensures that the functional or multivariate data coming into the analysis can be inhomogeneous across the domain of their arguments and this phenomenon does not influence the result of the analysis. For example, before the methods presented in this paper appeared, formal goodness-of-fit testing in spatial statistic was commonly based on the unscaled MAD test \citep{Ripley1981} or its non-graphical integrated counterpart \citep{Diggle1979}. However, the result of these tests is influenced by unequal variability of the test function across its domain leading in general to loss of power \citep{MyllymakiEtal2015, MyllymakiEtal2017}. 
...
A further advantage of the rank tests is that it allows one to give equal weights to the components fed in. Thus, the method is particularly well suited also for multiple testing with several univariate or functional test statistics \citep{MrkvickaEtal2017}, for constructing central regions jointly for various transformations of the functions \citep[for details see][]{DaiEtal2018}, and for combining various dimensions of multidimensional functions or various functional elements of multivariate functions. 
