...
The test output is shown in Figure~\ref{fig:trees_CSR} (left), which shows no evidence against CSR \citep[see more detailed description in][Section S4]{MyllymakiEtal2017}.
...
Figure~\ref{fig:necdfs} shows the empirical cumulative distribution functions obtained by the \fct{ecdf} of the \proglang{R} package \pkg{stats} \citep{R2020} for the heights of the 54 girls and 39 boys of the \code{growth} data \citep[see above, and][]{RamsaySilverman2006} at ages 10 (left) and 14 (right).
...
@
%
and plot the object returned by \fct{GET.necdf}. Figure~\ref{fig:necdfs_means_GET} shows the test output of the comparison of girls and boys at ages 10 (left) and 14 (right).
...
grid.arrange(p1, p2, nrow=1)
@
	\caption{Global envelope tests for comparison of the empirical cumulative distribution functions of heights of the girls and boys (see Figure~\ref{fig:necdfs}). Left: Age 10; Right: Age 14.}
...
@
%
The \code{fdata} object \code{poblenou[['nox']]} is automatically converted to the required format in \pkg{GET}. The observed differences together with the global envelope are shown in Figure~\ref{fig:graphfanova_logNOx}.
