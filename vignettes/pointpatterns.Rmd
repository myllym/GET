...
The examples utilize the R [@R2020] package spatstat [@spatstat2015] in addition to the GET package [@MyllymakiMrkvicka2020]. The envelope plots are produced by the use of the ggplot2 package [@Wickham2016]
```{r setup}
library("GET")
library("spatstat")
```
...
```{r csr, fig.width=4, fig.height=4}
...
The adjusted test of the GET package is described in Section 2.3 of @MyllymakiMrkvicka2020. The procedure was suggested by @BaddeleyEtal2017 and extended for global envelopes in GET [@MyllymakiMrkvicka2020].
...
```{r saplings_csr_L3, fig.width=4, fig.height=4}
...
```{r saplings_matclust2, fig.width=4, fig.height=4}
...
```{r saplings_matclust3, fig.width=4, fig.height=4}
