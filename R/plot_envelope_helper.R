...
#' @param ylab See \code{\link{plot.default}}.
#' @import ggplot2
two_envelopes_ggplot <- function(env1, env2, base_size = 15, main, ylim, xlab, ylab, curve_sets=NULL) {
    if(!any(class(env1) %in% c("fboxplot", "global_envelope")) |
       !any(class(env2) %in% c("fboxplot", "global_envelope"))) stop("env1 and/or env2 is not desired object type.\n")
