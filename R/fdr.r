...
#' @param pvalues A vector of p-values. An alternative input of curve_set.
#' @export
#' @examples
...
#' }
fdr_ATS <- function(curve_set, pvalues, alpha = 0.05,
                    alternative = c("two.sided", "less", "greater")) {
  if(missing(curve_set)) if(missing(pvalues)) stop("Either curve_set or pvalues need to be provided.")
  if(!missing(curve_set)) {
    ...
    pvalues <- vector(length=nr)
    for(i in 1:nr) {
      pranks <- calc_pointwiserank(data_and_sim_curves[,i])
      pvalues[i] <- estimate_p_value(x=-pranks[1], sim_vec=-pranks[-1], ties = 'conservative')
    }
  }
  else {
    nr <- length(pvalues)
  ...
  rej.sorted <- which(pvalues.sorted <= (1:nr)*((alpha/(1+alpha))/nr))
  r1 <- vlast(rej.sorted)
  pi0 <- 1
  if(length(rej.sorted) < 1) rej <- rep(FALSE, times=nr)
    ...
    rej.sorted <- which(pvalues.sorted <= (1:nr)*((alpha/pi0)/nr))
    r1 <- vlast(rej.sorted)
    rej <- pvalues <= pvalues.sorted[r1]
  }
  list(pvalues=pvalues, rej=rej, pi0=pi0)
}
